...
const IMG_BY_ASIN: Record<string, string> = {
  ...
  B01JSJFBNE: 'https://www.waterdropfilter.com/cdn/shop/files/ui-wd-k19-s-vis.png?v=1774504000',
...
const IMG_BY_PRODUCT_KEY: Record<string, string> = {
  ...
  'frizzlife pd1000-tam4': IMG_BY_ASIN.B0BK8ZRY2K,
  'waterdrop k19-s countertop ro': IMG_BY_ASIN.B0BHQRNGZ8,
  'aquatru classic': IMG_BY_ASIN.B0CQS3HQ8F,
  'frizzlife sk99': IMG_BY_ASIN.B084HW5BMT,
  'frizzlife mk99': IMG_BY_ASIN.B07ZY9RVN2,
  'epic smart shield': IMG_BY_ASIN.B076S1W5QY,
  'aquasana claryum 3-stage': IMG_BY_ASIN.B00CX8R5Q8,
  ...
  'waterdrop pitcher filter': IMG_BY_ASIN.B01JSJFBNE,
...
export function lookupFilterProductImage(pick: TopPickRow): string | undefined {
  ...
  const asin = extractAsin(pick.amazon);
...
export function attachProductImages(picks: TopPickRow[]): TopPickRow[] {
  return picks.map((pick) => {
    const img = lookupFilterProductImage(pick);
    return img ? { ...pick, img } : pick;
  });
}
