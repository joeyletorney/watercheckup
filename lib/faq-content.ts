export const FAQ_PAGE_URL = 'https://watercheckup.com/faq';
    ...
    items: [
      {
        ...
        a: 'WaterCheckup pulls from six authoritative data sources — five from the EPA and one from the Environmental Working Group (EWG). (1) EPA Violation Records — live violation history and system data for 150,000+ community water systems via the EPA Envirofacts API. (2) EPA PFAS Testing Data — federal PFAS monitoring results from 2023–2025 covering 6,151 water systems, the most comprehensive PFAS dataset ever collected. (3) EPA Enforcement History — inspection records, formal actions, and penalty data from the EPA enforcement database. (4) EPA Lead Tap Sampling — actual measured lead levels at the tap from EPA Lead & Copper Rule testing. (5) EPA Lead Pipe Inventory — federal lead service line inventory data, which all US public water systems were required to publish as of October 2024. (6) EWG Health Guidelines — the Environmental Working Group\'s independent health benchmarks, which are stricter than EPA legal limits and used to flag contaminants that are legal but still a concern. Your ZIP code is matched to your specific water system and all available data is combined into a single free report.',
      },
      {
        q: 'How is the Water Safety Score calculated?',
        a: 'The Water Safety Score (0–88) blends EPA violation history, PFAS UCMR5 detections, hardness, and contaminants above health guidelines — not legal compliance alone. City and ZIP lookups use the same formula, so you see one grade everywhere. Scores in the A range (~80+) reflect relatively clean profiles; the B–C range flags hardness, PFAS, or metals worth filtering; D and below indicate stacked risks or MCL violations. A higher score still does not mean you should skip filtration — it means fewer documented concerns in the federal data we show, not contaminant-free water at your tap.',
        links: [{ href: '/methodology', label: 'Full data sources & scoring methodology' }],
      },
      {
        q: 'Why is the highest score 88, not 100?',
        a: 'We cap the scale at 88 because no public water system is "perfect" in real-world use. Even excellent utilities add chlorine or chloramine, form disinfection byproducts, and may show trace detections in federal monitoring. The score also reflects your public water system — not what happens inside your home. Water can pick up lead, copper, or sediment from service lines and building plumbing on the way to your faucet, which annual utility reports cannot fully predict. Reserving the top of the scale avoids implying that an 88/88 report means you never need a filter or home testing.',
      },
      {
      ...
      {
        q: 'Is WaterCheckup affiliated with the EPA or my public water system?',
        a: 'No. WaterCheckup is an independent consumer tool. We aggregate publicly available EPA and other third-party datasets to make them easier to read; we do not speak for the EPA, your state regulator, or your water provider. Always confirm critical decisions (health, pregnancy, infants, legal compliance) with your public water system, a certified lab, or a licensed professional. Official enforcement and billing questions belong to your water system directly.',
      },
    ],
  },
];
...
export function faqSchemaEntities(): { name: string; text: string }[] {
  return FAQ_SECTIONS.flatMap((section) =>
    section.items.map((item) => ({
      name: item.q,
      text: item.links?.length
        ? `${item.a} More: https://watercheckup.com${item.links[0].href}`
        : item.a,
    }))
  );
}
